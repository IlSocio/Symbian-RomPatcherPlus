...
//
	...
	if((KSizeOfSegment/2) < aLength) 
		{
		// Don't cache anything more than a page long!
...
//
	...
	if( IsOverlapping( hExt.iDirTreeOffset, h.iDirTreeSize,
...
//
	...
	if (aPos>=iSize)
		{
		aMessage.WriteL(0,TPtrC8(NULL,0),0);
		aLength=0;
		}
	else
		{
		...
		__PRINT2(_L("ROFS::ReadL() pos=%d len=%d"),aPos, len);
		RofsMount().CacheReadL( aPos + iMediaBase, len, aDes, 0, aMessage );
#else
		TInt r = RofsMount().LocalDrive()->Read( aPos + iMediaBase, len, aDes, aMessage.Handle(),0) ;
		User::LeaveIfError( r );
#endif
		aLength=len;
		}
...
TInt CRofs::DriveList(TDriveList& aList) const
	{
	TInt maxLocalDrives = iTotalSupportedDrives;

	Mem::FillZ(&aList[0],aList.MaxSize());
	TLocalDriveCapsV2Buf localDriveCaps;
	for (TInt i=0; i<maxLocalDrives; i++)
		{
		TInt r=GetLocalDrive(i).Caps(localDriveCaps);
		if( i == maxLocalDrives - 1 )
			{
			// special case, we fill this in for drives A: and Y:
			aList[EDriveA] = (r==KErrNone) ? (TUint8)localDriveCaps().iDriveAtt : (TUint8)(KDriveAttLocal|KDriveAttRemovable);
			aList[EDriveY] = aList[EDriveA];
			}
		else
			{
			aList[i+KFirstLocalDrive]=(r==KErrNone) ? (TUint8)localDriveCaps().iDriveAtt : (TUint8)(KDriveAttLocal|KDriveAttRemovable);
			}
		}
	aList[EDriveB]=KDriveAbsent;
	aList[KFirstLocalDrive + maxLocalDrives - 1] = KDriveAbsent;
	return(KErrNone);
	}
...
//
	{
    TLocalDriveCapsV2Buf localDriveCaps;
	GetLocalDrive(aDriveNumber).Caps(localDriveCaps);
