// SC_COMP.CPP
//
// Copyright (c) 2003-2006 Symbian Ltd.  All rights reserved.
//
...
LOCAL_C void Fault(TCompFault aFault)
//
...
//
	{
	__PRINT(_L("CCompFileSystem()"));
	__DECLARE_NAME(_S("CCompFileSystem"));
	}
...
//
	{
	__PRINT1(_L("~CCompFileSystem() at 0x%x"),this);
	if (iMount)
		{
...
//
	{
	__PRINT(_L("Installing composite file system..."));
...
//
	{
	__PRINT(_L("CCompFileSystem::NewMountL()"));
	...
	__PRINT1(_L("mount at 0x%x"),iMount);
...
//
	...
	for(TInt idx=0; idx<count; idx++)
		{
		TRAPD(err, pDir->iDirs.Append(iMount->iMounts[idx].iFs->NewDirL()));
		if(err != KErrNone)
			{
			pDir->Close();
			User::Leave(err);
...
//
{
	...
	iMounts.Append(TCompMount(romFs,romMount));
}
...
//
 ...
 TInt CCompMountCB::AddFsToCompositeMount(CFileSystem* aFileSystem)
	//
	// Mounts a media with the provided filesystem and adds to the composite
	// mount.  Local drive mapping for this mount's drive (CompFS's) should
	// be altered to indicate the local drive to be added.
 	//
 	{
	...
	if (err==KErrNone)
		{
		newMount->SetDrive(&Drive());		
		TRAP(err,newMount->MountL(EFalse));
		if (err==KErrNone)
			{
			iMounts.Append(TCompMount(aFileSystem,newMount));
			return (KErrNone);
			}
		}
...
LOCAL_C TInt InitFile(CFileCB* aCompFile, CFileCB* aTrueFile,CMountCB* aRealMount,TUint8 aQueOffset)
//
// Initialise the true file object with values set in the composite file object
// by TDrive
//
	{
	TRAPD(r,aTrueFile->InitL(&aCompFile->Drive(),&aCompFile->Drive(),aCompFile->FileName().Des().AllocL(), new(ELeave) RArray<SFileShareLock>(KFileShareLockGranularity,_FOFF(SFileShareLock,posLow))) );
	if(r!=KErrNone)
		return(r);
...
LOCAL_C void SetAtt(CDirCB* aDir,TUint8 aOffset,TUint aValue)
//
//
//
	{	
...
LOCAL_C void SetUid(CDirCB* aDir,TUint8 aOffset,TUidType aValue)
//
//
//
	{
...
//
	{
	__PRINT1(_L("~CCompFileCB() at 0x%x"),this);
	if(TrueFile())
		TrueFile()->Close();
	}
...
//
	{
	__PRINT1(_L("~CCompDirCB at 0x%x"),this);
